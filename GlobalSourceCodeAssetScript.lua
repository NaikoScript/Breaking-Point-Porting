...
if UI.inv.sectionframes:GetChildren()[SkinType]:FindFirstChild("GridOrder") then
    ...
    local Tool
    task.spawn(function()
        if SwapTheSkinLocally == true and SkinType == 4 and SkinName:sub(1,2) == "/e" then
            LocalPlayer.Chatted:Connect(function(msg)
                if msg:lower() == SkinName:lower() then
                    if LocalPlayer.Character ~= nil and LocalPlayer.Character:FindFirstChildOfClass("Humanoid") and LocalPlayer.Character:FindFirstChildOfClass("Humanoid").Health > 0 then
                       local Animation = LocalPlayer.Character:FindFirstChildOfClass("Humanoid"):LoadAnimation(Data[1])
